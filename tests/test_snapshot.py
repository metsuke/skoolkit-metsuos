...
from skoolkit.snapshot import Memory, Snapshot, Z80, get_snapshot, write_snapshot, SnapshotError
...
class MemoryTest(SkoolKitTestCase):
    ...
    def test_48k_banks(self):
        memory = Memory(banks={b: [b] * 0x4000 for b in (1, 2, 5)})
        for bank, addr, value in ((None, 0x0002, 0x54), (5, 0x421B, 0x08), (1, 0x89F8, 0xF4), (2, 0xEA1A, 0x29)):
            if bank is not None:
    ...
    def test_128k_address_range(self):
        memory = Memory([0] * 0x20000, page=3)
        start = 0xA135
    ...
    def test_128k_address_range_with_step(self):
        memory = Memory([0] * 0x20000, page=4)
        start = 0x4174
